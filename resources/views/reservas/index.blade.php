    ...
    <tbody>
        @foreach($Reservas as $reserva)
            <tr>
                ...
                <td>
                    <a class="btn btn-primary btn-sm" href="{{ route('editar_reserva', ['id' => $reserva->id]) }}" title="Editar reserva">
                        <i class="fas fa-edit"></i>
                    </a>
                </td>
                <td>
                    <form action="{{ route('eliminar_reserva', ['id' => $reserva->id, 'id' => $reserva->id]) }}" method="POST">
                    @csrf @method('DELETE')
